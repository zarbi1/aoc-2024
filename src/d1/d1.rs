...
pub fn compute_diff(file_path: &str) -> Result<i32, Error> {
    let lists = compute_two_lists(file_path).unwrap_or_else(|error| panic!("Error: {:?}", error));
...
pub fn compute_sim(file_path: &str) -> Result<i32, Error> {
    //first we get the content of th input
    let mut content = read_input(file_path).unwrap_or_else(|error| {
        panic!("Could not extract text: {:?}", error);
    });
